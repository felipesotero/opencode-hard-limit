...
function decorateResult(result, stale, receivedAt) {
  if (!result || typeof result !== "object") return result;
  if (stale) return { ...result, stale: true, receivedAt };
...
export async function readWeekly({
  ...
  if (cachedResult && (backoffActive || hasFreshCache(entry, now, cacheTtlMs) || hasMinRefresh(entry, now, minRefreshIntervalMs))) {
    return decorateResult(cachedResult, stale, entry.at);
  }
  ...
  const pendingFetch = (async () => {
    ...
    if (!result || result.ok !== true) {
      if (result?.errorKind === "ratelimit") {
        ...
        if (entry?.result && entry.result.ok === true) {
          ...
          return decorateResult(entry.result, true, entry.at);
        }
        ...
        return decorateResult(errorResult, false, now);
      }
    ...
    return decorateResult(result, false, now);
  })().finally(() => {
    inflight.delete(key);
  });
