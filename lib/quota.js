...
async function fetchAnthropic({ window, timeoutMs }) {
  ...
  if (!res.ok) return { ok: false, error: `Anthropic API error ${res.status}: ${res.bodySnippet}` };
...
async function fetchOpenAI({ window, timeoutMs }) {
  ...
  if (!res.ok) return { ok: false, error: `OpenAI API error ${res.status}: ${res.bodySnippet}` };
