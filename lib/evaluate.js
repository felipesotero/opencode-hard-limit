...
import { parseResetAt, windowDurationMs } from "./reset.js";
...
export function evaluate(_quotaProvider, res, cfg, now = Date.now()) {
  if (res.ok) {
    ...
    // Exemption: "window reset since the LKG reading". Also bound how far in
    // the past that reset can be — a reset more than one window-length ago
    // is stale/garbage data, so the conservative choice is to keep the
    // stale-failsafe armed (fail toward blocking) rather than exempt it.
    const parsedResetAt = parseResetAt(res.resetAt);
    const resetWindowDuration = windowDurationMs(res.window ?? cfg.window);
    const windowAlreadyReset =
      parsedResetAt !== null && parsedResetAt < now && now - parsedResetAt <= resetWindowDuration;
